...
func server() {
	...
	listener, err := listenConfig.Listen(context.TODO(), "tcp", *strAddrMain)
...
func serverHole() {
	...
	// listener, err := net.ListenTCP("tcp", addr)
	listener, err := listenConfig.Listen(context.TODO(), "tcp", *strAddrComm)
	checkErr(err)
	defer listener.Close()
...
func cmdSend(strAddrClient, content string) {
	...
	conn, err = d.Dial("tcp", strAddrClient)
	if err != nil {
		fmt.Println(err)
		return
	}
